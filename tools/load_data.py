...
class LoadData:
    ...
    def load_choujiang_file(self,entity_map_dict):
        ...
        for i, line in enumerate(open(choujiang_file)):
            dt, hostnum, uid, item_id,hist_buy_list,label, effect_beast_mp_left, effect_hp_left, effect_beast_hp_left, cur_lingqi, effect_mp_left,beast_mp_seg,hp_seg,best_hp_seg,lingqi_seg,mp_seg=line.strip().split('\t')

            if item_id in entity_map_dict.keys():
                hostnum=int(hostnum)
                uid=int(uid)
                label=int(label)
                ...
                # TODO 目前remain_list在KG里面是没有node的，需要恢复一下，但是不要有node
